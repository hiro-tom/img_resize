                                        ...
                                        try {
                                                ...
                                                panel.innerHTML = `
                                                    ...
                                                    <div class=\"section\">
                                                        ...
                                                        <label>SFTPダウンロード元フォルダー (remote_dir)</label>
                                                        <input id=\"f_remote\" value=\"${s.remote_dir || ''}\" />
                                                        <label>SFTPアップロード先フォルダー (remote_output_dir)</label>
                                                        <input id=\"f_remote_output\" value=\"${s.remote_output_dir || ''}\" placeholder=\"任意: SFTP接続先への出力先\" />
                                                        <label>SFTPダウンロード先ローカルフォルダー (local_dir)</label>
                                                        <input id=\"f_local\" value=\"${s.local_dir || ''}\" />
                                                        <label>画像圧縮処理先ローカルフォルダー (compress_output_dir)</label>
...
def _run_upload() -> None:
    ...
    settings = SftpSettings(**settings_data)
    _log("DEBUG", f"ロード設定: compress_output_dir={settings.compress_output_dir}, remote_output_dir={settings.remote_output_dir}, upload_interval_seconds={settings.upload_interval_seconds}")
    ...
    try:
        # Check if compress_output_dir exists and has files
        import os
        if not os.path.exists(settings.compress_output_dir):
            _log("WARN", f"アップロード対象フォルダーが存在しません: {settings.compress_output_dir}")
            return
        
        # Count files in compress_output_dir
        file_count = 0
        for root, dirs, files in os.walk(settings.compress_output_dir):
            file_count += len(files)
        
        _log("INFO", f"アップロード対象フォルダーに {file_count} 個のファイルがあります: {settings.compress_output_dir}")
...
@app.get("/upload/diagnose")
def diagnose_upload(user=Depends(require_auth)):
    ...
    settings_data = db.load_settings()
    if not settings_data:
        return {"error": "設定が未構成です"}

    settings = SftpSettings(**settings_data)
    
    # Check settings
    compress_dir_exists = False
    compress_files_count = 0
    compress_dir_contents = []
    
    if settings.compress_output_dir and os.path.exists(settings.compress_output_dir):
        compress_dir_exists = True
        for root, dirs, files in os.walk(settings.compress_output_dir):
            for file in files:
                file_path = os.path.join(root, file)
                file_size = os.path.getsize(file_path)
                rel_path = os.path.relpath(file_path, settings.compress_output_dir)
                compress_files_count += 1
                compress_dir_contents.append({
                    "name": rel_path,
                    "size": file_size
                })
    
    return {
        "compress_output_dir": settings.compress_output_dir,
        "compress_dir_exists": compress_dir_exists,
        "compress_files_count": compress_files_count,
        "compress_dir_contents": compress_dir_contents[:10],  # First 10 files
        "remote_output_dir": settings.remote_output_dir,
        "remote_output_dir_is_set": bool(settings.remote_output_dir),
        "upload_interval_seconds": settings.upload_interval_seconds,
        "upload_running": _upload_running,
    }
