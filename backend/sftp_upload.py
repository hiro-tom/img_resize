...
def upload_file(
    ...
    def _log(level: str, message: str, detail: Optional[str] = None):
        ...
        # Also log to stdout for debugging (without print to avoid stream issues)
        import sys
        if detail:
            msg = f"[{level}] {message} - {detail}"
        else:
            msg = f"[{level}] {message}"
        try:
    ...
    try:
        ...
        try:
            ...
            _log("DEBUG", f"[リモートファイル確認] {remote_file}: {remote_stat.st_size} bytes")
        except Exception as e:
...
def upload_folder(
    ...
    _log("DEBUG", f"アップロード処理: local_dir={local_dir}, remote_dir={remote_dir}")
    _log("DEBUG", f"ローカルディレクトリ存在確認: {local_path.exists()}")
    ...
    for root, dirs, files in os.walk(local_dir):
        ...
        for filename in files:
            ...
            _log("DEBUG", f"アップロード対象ファイル検出: {rel_path}")

    if not files_to_upload:
        _log("WARN", "アップロード対象のファイルがありません")
        return stats
